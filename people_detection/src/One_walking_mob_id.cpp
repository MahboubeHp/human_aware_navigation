...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    // Move back and forth between points A and B
    person.position.x = x_start + delta_t * person.velocity.x;

    // Change the sign of velocity when reaching either end
    if (person.position.x >= x_end || person.position.x <= x_start)
    {
      vel = -vel;
      start = ros::Time::now(); // Reset start time
    }

    person.position.y = y + delta_t * person.velocity.y;
    
    person.name = "classified_person";
    person.mob_id = 1.0;
